namespace Rubns.Application.UseCases.Scrapper.Queries
{
        ...
        : IGetMetasUseCase
    {
        ...
        private readonly IGetMetasOutputPort _metasOutputPort = metasOutputPort;
        ...
        public async Task ExecuteAsync(string url)
        {
            ...
            try
            {
                ...
                var exists = await _qRRepositoryEFC.FindBySlugAsync(slug);
                if (string.IsNullOrEmpty(exists))
                {
                    metas.Add("slug", slug);
                }

                await _metasOutputPort.Success(metas);
